...
class SignalProp(torch.nn.Module):
    ...
    def forward(self, g, ts, nf, groundtruth=False):
        ...
        with g.local_scope():
            g.ndata['nf'] = nf
            g.ndata['new_nf'] = torch.zeros(g.num_nodes(), self.out_nf, device='cpu', dtype=nf.dtype)
...
class TimingGCN(torch.nn.Module):
    ...
    def forward(self, g, ts, groundtruth=False):
        try:
            ...
            return torch.zeros((num_nodes, 4)), torch.zeros((num_nodes, 4)), {}
...
#             x = layer(g, torch.cat([x, g.ndata['nf']], dim=1)) + x
#         x = self.layern(g, x)
#         return x
